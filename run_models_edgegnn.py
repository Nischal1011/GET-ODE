'''
Edge-GNN baseline: same generative ODE/decoder/VAE loss as LG-ODE (lib/create_latent_ode_model.py
imports reused unchanged, see lib/create_edgegnn_model.py), only the encoder differs
(lib/edge_gnn_models.py). Uses the same corrected dataloaders (CorrectedParseData /
IEEE39ParseData, see lib/corrected_dataLoader.py) as run_models_corrected.py for a fair,
apples-to-apples comparison: same data, splits, normalization, and observation masks.
'''
import os
import sys
...
from lib.create_edgegnn_model import create_EdgeGNN_model
from lib.utils import compute_loss_all_batches

parser = argparse.ArgumentParser('Edge-GNN baseline')
parser.add_argument('--n-balls', type=int, default=5)
parser.add_argument('--niters', type=int, default=50)
parser.add_argument('--lr', type=float, default=5e-4)
parser.add_argument('-b', '--batch-size', type=int, default=256)
parser.add_argument('--save', type=str, default='experiments_edgegnn/')
...
parser.add_argument('--z0-encoder', type=str, default='EdgeGNN')
...
if __name__ == '__main__':
    ...
    model = create_EdgeGNN_model(args, input_dim, z0_prior, obsrv_std, device)
