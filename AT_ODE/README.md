...
### Local acceleration and early stopping

`run_models_gpu.py` uses `--device auto` by default: CUDA is preferred when
available, followed by Apple Metal (`mps`), then CPU. Use `--device mps` to
require the Apple GPU or `--device cpu` for a CPU-only run.

Training reserves 10% of the training graphs for deterministic validation and
stops after 15 validation checks without an MSE improvement of at least `1e-5`.
These defaults can be changed with `--validation-fraction`,
`--early-stopping-patience`, and `--early-stopping-min-delta`. A patience of
zero disables early stopping. Checkpoint selection uses validation MSE; the
held-out test set is evaluated once after the best validation weights are
restored.

```bash
python run_models_gpu.py --data motion --device mps --niters 100 \
  --validation-fraction 0.1 --early-stopping-patience 15 \
