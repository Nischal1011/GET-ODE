...
class ParseData(object):
    ...
    def load_data(self,sample_percent,batch_size,data_type="train"):
        ...
        # Loading Data
        loc = np.load(self.dataset_path + '/loc_' + file_data_type + self.suffix + '.npy', allow_pickle=True
)[:cut_num]
        vel_path = self.dataset_path + '/vel_' + file_data_type + self.suffix + '.npy'
        if os.path.exists(vel_path):
            vel = np.load(vel_path, allow_pickle=True)[:cut_num]
            self.has_vel = True
        else:
            vel = None
            self.has_vel = False

        edges = np.load(self.dataset_path + '/edges_' + file_data_type + self.suffix + '.npy', allow_pickle=True
)[:cut_num]  # [500,5,5]
        times = np.load(self.dataset_path + '/times_' + file_data_type + self.suffix + '.npy', allow_pickle=True
)[:cut_num]  # 【500，5]

        if data_type in {"train", "val"}:
            validation_fraction = float(getattr(self.args, "validation_fraction", 0.1))
            if not 0.0 < validation_fraction < 1.0:
                raise ValueError("validation_fraction must be between 0 and 1")

            total_graphs = loc.shape[0]
            validation_graphs = max(1, int(round(total_graphs * validation_fraction)))
            if validation_graphs >= total_graphs:
                raise ValueError("validation_fraction leaves no graphs for training")

            split_rng = np.random.RandomState(self.random_seed)
            split_indices = split_rng.permutation(total_graphs)
            selected = (
                split_indices[:-validation_graphs]
                if data_type == "train"
                else split_indices[-validation_graphs:]
            )
            loc = loc[selected]
            if vel is not None:
                vel = vel[selected]
        ...
        loc_dim = loc[0][0][0].shape[0]
        if self.has_vel:
            vel_dim = vel[0][0][0].shape[0]
        else:
            vel_dim = 0

        self.num_graph = loc.shape[0]
        self.num_atoms = loc.shape[1]
        self.feature = loc_dim + vel_dim

        if self.suffix in ["_springs5", "_charged5", "_mocap35", "_pems08"]:

            # Normalize features to [-1, 1], across test and raw dataset
            ...
            if self.max_loc is None:
                ...
                # -----------------------------------------
                # Shard only TRAIN data after normalization
                # -----------------------------------------
        ...
        if world_size > 1 and data_type == "train":

            total_graphs = loc.shape[0]
            graphs_per_rank = total_graphs // world_size

            start = rank * graphs_per_rank
            end = (rank + 1) * graphs_per_rank if rank != world_size - 1 else total_graphs
        ...
        # -----------------------------------------
        # Only rank 0 keeps test data
        # -----------------------------------------
        if world_size > 1 and data_type in {"val", "test"} and rank != 0:
            return None, None, None, 0

        print("number graph in   "+data_type+"   is %d" % self.num_graph)
        print("number atoms in   " + data_type + "   is %d" % self.num_atoms)

        # split raw w.r.t interp and extrap, also normalize times
    ...
    def interp_extrap(self,loc,vel,times,mode,data_type):
        ...
        vel_observed = None if vel is None else np.ones_like(vel)

        times_observed = np.ones_like(times)
        if mode =="interp":
            if data_type== "test":
                # get ride of the extra nodes in testing raw.
            ...
            vel_extrap = None if vel is None else np.ones_like(vel)
    ...
    def split_data(self,loc,vel,times):
        ...
        for i, loc_series in enumerate(loc_list):
            ...
            if self.has_vel:
                ...
                feat = loc_series[preserved_idx]

            feature_observe[:len(times_list[i][preserved_idx])] = feat
            ...
            # for decoder raw, padding and mask
            ...
            if self.has_vel:
                feat = np.concatenate((loc_series, vel_list[i]), axis=1)
            else:
                feat = loc_series

            feature_predict[:len(times_list[i])] = feat
    ...
    def transfer_data(self,loc, vel, edges, times, time_begin=0):
        ...
        for i in iterator:
            ...
            data_per_graph, edge_data, edge_size = self.transfer_one_graph(
                loc[i], vel_i, edges[i], times[i],
    ...
    def variable_time_collate_fn_activity(self,batch):
        """
        Expects a batch of time series raw in the form of (record_id, tt, vals, mask, labels) where
            - record_id is a patient id
            - tt is a 1-dimensional tensor containing T time values of observations.
            - vals is a (T, D) tensor containing observed values for D variables.
            - mask is a (T, D) tensor containing 1 where values were observed and 0 otherwise. Since in human dataset, it join the raw of four tags (belt, chest, ankles) into a single time series
    ...
    def normalize_features(self,inputs, num_balls):
        ...
        :param inputs: [num-raw, num-ball,(timestamps,2)]
        :return:
        '''
