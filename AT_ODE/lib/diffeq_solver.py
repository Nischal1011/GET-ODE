...
class DiffeqSolver(nn.Module):
    ...
    def forward(self, first_point, time_steps_to_predict, graph, backwards = False):
        ...
        adjoint_params += self.ode_func.edge_posterior_adjoint_tensors
        pred_y = odeint(
            self.ode_func,
            first_point_augumented,
            time_steps_to_predict,
            rtol=self.odeint_rtol,
            atol=self.odeint_atol,
            method=self.ode_method,
            adjoint_params=adjoint_params,
        ) #[time_length, n_sample*b,n_ball, d]
...
class GraphODEFunc(nn.Module):
    def __init__(self, ode_func_net,  device = torch.device("cpu")):
        ...
        self.nfe = 0
        self.edge_posterior_adjoint_tensors = ()

    def set_edge_posterior(self, rel_type_provider, adjoint_tensors=()):
        for layer in self.ode_func_net.gcs:
            if hasattr(layer.base_conv, "rel_type_provider"):
                layer.base_conv.rel_type_provider = rel_type_provider
        self.edge_posterior_adjoint_tensors = tuple(adjoint_tensors)

    def forward(self, t_local, z, backwards = False):
        """
        Perform one step in solving ODE. Given current raw point y and current time point t_local, returns gradient dy/dt at this time point
        ...
        self.last_derivative_norm = grad.detach().float().norm(dim=-1).mean()
