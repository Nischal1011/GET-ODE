'''
AT-LG-ODE: LG-ODE with the encoder's relational attention transported forward in time to
reweight the graph ODE's aggregation (see lib/at_*.py). Mirrors run_models.py exactly except
for model/data-loader construction and two extra hyperparameters (--lambda-init,
--learnable-lambda), so results are directly comparable to a run_models.py run with the same
flags.
'''
import os
import sys
from lib.at_new_dataLoader import ATParseData
...
from lib.create_at_latent_ode_model import create_AT_LatentODE_model
from lib.utils import compute_loss_all_batches

parser = argparse.ArgumentParser('AT-Latent Graph ODE')
...
parser.add_argument('--save', type=str, default='experiments_at/', help="Path for save checkpoints")
...
if __name__ == '__main__':
    ...
    dataloader = ATParseData(args.dataset, suffix=args.suffix, mode=args.mode, args=args)
    ...
    model = create_AT_LatentODE_model(args, input_dim, z0_prior, obsrv_std, device)
    ...
    def train_epoch(epo):
        ...
        message_train = 'Epoch {:04d} [Train seq (cond on sampled tp)] | Loss {:.6f} | MSE {:.6F} | Likelihood {:.6f} | KL fp {:.4f} | FP STD {:.4f}| lambda {:.4f}|'.format(
            epo,
            np.mean(loss_list), np.mean(mse_list), np.mean(likelihood_list),
            np.mean(kl_first_p_list), np.mean(std_first_p_list), model.transport.lam.item())
